...
def SplitFitParam(df, param_col_name):
...
def MergeDrugCells(df_drug_curves, df_cells, df_drug_properties, splitting_needed = False, param_col_name = "fitting_param",
                   save_CCL_properties = False, _FOLDER_to_save = None):
    ...
    common_cells_drug = list(set(np.array(df_cells.columns[1:], dtype="int"))& set(df_drug_curves["COSMIC_ID"].values))
    ...
    df_drug_curves = pd.merge(left=df_drug_curves, right = df_drug_properties, on = "DRUG_ID") 
    
    if splitting_needed:
        df_drug_curves = SplitFitParam(df_drug_curves, param_col_name)
...
def SplitTrainTestFor10Drugs(df, train_ratio = 0.8):
    ...
    print("Number of drug profiles not covered:", df.shape[0] - train.shape[0] - test.shape[0])
